...
function Booktrainpage({train,trainId,onClose ,searchParams,arrivalTime}) {
...
const [pincode, setPincode] = useState('');
  ...
  const calculateTotalCost = (noOfTickets, trainPrice) => {
    return noOfTickets * trainPrice;
  };
  ...
  return (
    <div>
    <div style={{ height: "550px" ,width:"1500px",marginLeft:"250px"}} className='ticket1'>
    ...
    <p className='ticket1Aba'>Prices may change due to dynamic pricing by Railways.</p>
    ...
    <div className='ticket2' style={{width:"1400px",position:"relative",left:"200px"}}>
    {bookingData&&(
      ...
      <p className='ticket2Aaa'>{bookingData.status}</p>
        ...
        <h3>Train details</h3>
        <p>{bookingData.train.trainName}
        {bookingData.train.trainNumber}</p>
        <p> From :{bookingData.train.source}--
        To :{bookingData.train.destination}</p>
        <p>Departure : {bookingData.start_date} Time :{bookingData.train.departureTime}</p>
        <p>arrival : {bookingData.end_date} Time :{bookingData.train.arrivalTime}</p>
        </div>
        ...
        <div className='bookfinalbaba' style={{display:"flex"}}>
          ...
          <p className='headingoffare3a' style={{height: '50px', width: '265px'}}>₹ {calculateTotalCost(noOfTickets, train.fare)}</p>
          <p className='headingoffare3a' style={{height: '50px', width: '265px', borderBottom: '2px solid grey'}}>₹140</p>
          <p className='headingoffare5a' style={{height: '50px', width: '265px'}}>{calculateTotalCost(noOfTickets, train.fare)+140}</p>
          </div>
